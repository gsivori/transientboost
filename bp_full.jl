...
function run_bpfull(sim_length=10000.0,pseed::Int64=1919,bp::Float64=60.0,bar::Bool=true,pretrained_w::Array{Float64,2}=zeros(2000,1),pretrained_κ::Float64=3.75)
    load_mat = false
    adapt = true
    jitter_checks = false
    rng = StableRNG(pseed)
    ...
    pf = 0.03 # current factor NMDA-to-Ca (a.u.)
    NMDAratio = 0.75 # 75% of exc input is dual AMPA/NMDA synapse
    ...
    n_pats = 3
    pats = []
    pats = generate_input_pats(rng,n_in,pat_width,n_pats,poiss_rate,sim_δt)
    ...
    #AMPA/NMDA synapses
    AMPAids = [x for (x,j) in enumerate(w) if j > 0]
    NMDAsyns = Int(round(length(AMPAids)*NMDAratio))
    NMDAids = sort(sample(AMPAids,NMDAsyns))
    ...
    input_mat, input_pat = generate_input_mat(rng,n_in, sim_steps, pats, pat_width, poiss_rate, sim_δt)
    ...
    for ti = 1:n_steps
            ...
            if active
                if rand(rng) > p_fail
                    if w[index] < 0
                        ...
                        g_E_prev += w[index]
                        (index in NMDAids) && (g_NMDA_prev += w[index])
                    end
                else
        ...
        dv_dend = (Icsd + ge*(Ve_r - v_dend) + gi*(Vi_r - v_dend) + g_ld*(Vd_l - v_dend) + Ica + Inmda)/C_dend
    ...
    return spks,ns,o_v_dend,o_v,(o_ge,o_gi,o_gcsd,o_gnmda,o_Ica,o_Ca,o_Inmda, o_Icsd),isis,o_cvs,o_pi,(presyns,input_mat,input_pat,pats),(w_init,w,o_P),(o_κ,o_ϐ)
end 
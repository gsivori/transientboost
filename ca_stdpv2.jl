...
function run_ca_stdpV2(reps::Int64=60,Ipulse::Float64=500.0,t_delay::Int64=0,n::Int64=1,in_freq::Float64=1.0,seed::Int64=2)
    rng = StableRNG(seed)
    ...
    τ_ref = 3.0 # absolute refractory period (ms) #2.
    g_cds = 108.0 # leak across comparments (nS) (coupling:0.9) g_cds/(g_cds+g_ls)
    g_csdr = 2.0 # leak across comparments (nS) (coupling:0.167) g_csdr/(g_csdr+g_ld)
    ...
    m_cs = 60.0 # back-propagation post-spike modulation (nS)

    #NMDA synaptic parameters
    pf = 0.05 # current factor Inmda-to-Ica (a.u.)
    ...
    Φ = 0.15 # Ca2+ current drive (mM/s.pA)
    ...
    len_of_pulse = 12 #times 2 for actual length
    #t_delay = tpre-tpost
    for j in range(Tno_act,stop=n_steps-1)# for output & input
        if j % Tperiod == 0
            ...
            if n == 1
                I_pulse[j-len_of_pulse:j] .= Ipulse
            end
            if n == 2
                spike_delay = 100
                I_pulse[j-len_of_pulse-spike_delay:j-spike_delay+len_of_pulse] .= Ipulse
                I_pulse[j-len_of_pulse:j] .= Ipulse
            end
        end
    end
    #synaptic input strength parameters
    κ = 1.0 # normalizing constant　(not present in single-synapse scenario)
    ...
    return rel_chg, (spks,in_spks), (o_CA,o_μCa,o_gain,o_At)
end

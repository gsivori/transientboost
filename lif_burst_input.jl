...
function run_spk(sim_length=10000.0,pseed::Int64=1919,bursting_percent::Float64=0.1,bar::Bool=true)
    load_mat = false 
    adapt = true
    rng = StableRNG(pseed)
    #cell parameters
    ...
    if bursting_percent > 0.0
        for each in 1:n_pats
            inject_bursts_replace(rng,pats[each],bursting_percent)
        end
    end
    #newpat = generate_single_input_pat(rng,n_in,pat_width,in_rate,sim_δt) #not used but to match rng with other results

    #synaptic input strength parameters
    w = rand(rng,Normal(),n_in)
    κ = 3.5 # # normalizing constant
